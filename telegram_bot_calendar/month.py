...
class MonthTelegramCalendar(TelegramCalendar):
    first_step = YEAR

    def __init__(self, calendar_id=0, current_date=None, additional_buttons=None, locale='en',
                 min_date=None,
                 max_date=None, telethon=False, **kwargs):
        super(MonthTelegramCalendar, self).__init__(calendar_id, current_date=current_date,
    ...
    def _build(self, step=None, **kwargs):
        ...
        if step == YEAR:
            ...
            self.current_date = date(self.current_date.year, self.current_date.month, 2)
    ...
    def _process(self, call_data, *args, **kwargs):
        ...
        if params['action'] == SELECT:
            if step in STEPS:
                ...
                if(self.current_date.day == 2):
                    return self.current_date, self._keyboard, STEPS[step]
                else:
    ...
    def _get_period(self, step, start, diff, *args, **kwargs):
        if step != DAY:
            return super(MonthTelegramCalendar, self)._get_period(step, start, diff, *args, **kwargs)
